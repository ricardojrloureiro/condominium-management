...
void Corporation::removeCondominium(Condominium cond){
	// procurar o metodo que procura no vetor e associa √† possi√ß√£o seguinte passado a dele
}
...
void Corporation::loadProperties(int condominiumid) {
	...
	while (file.good())
	{
		...
		if (lineNumber > 0) {
			...
			//REMOVER ESTA MERDA PORQUE ISTO … DO TIPO 0 YOYOYO TIPO 0 NO ES WORKING
			if(type==0){
				Apartment apartment(address, cost);
				condominiums[searchCondominiumId(condominiumid)].addProperty(apartment);
			}
			//COME«A APARTIR DAQUI OH FILHOTE MANINHO YOYOYOYO
			if (type==1) {
				Apartment apartment(address, cost);
				condominiums[searchCondominiumId(condominiumid)].addProperty(apartment);
			} else if (type==2) {
